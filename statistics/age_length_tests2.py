...
import numpy as np
from stattests import mannwhitneyu
...
for ind1, tp1 in enumerate(types):
    for ind2, tp2 in enumerate(types):
        _, birthtime_output[ind1, ind2] = mannwhitneyu(birthtime_data[ind1], birthtime_data[ind2])
        _, length_output[ind1, ind2] = mannwhitneyu(length_data[ind1], length_data[ind2])
