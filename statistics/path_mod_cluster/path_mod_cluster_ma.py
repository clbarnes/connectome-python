from path_mod_cluster.shared import av_path_length
...
for source, connectome in C.items():
    ...
    G = nx.DiGraph(connectome.compose('Synapse', 'GapJunction', 'Monoamine'))
    ...
    for G in iterate_adj_mats(os.path.join(data_root, 'randoms_no-np', source, 'whole.mat')):
...
json.dump(output, open('path_cluster_ma_output.json', 'w'), sort_keys=True, indent=4)
